...
@section('content')
    ...
    <div class="card col-12">
        <div class="card-header d-flex flex-row justify-content-between">
            <div class="col-6"><h4>Data SPD</h4></div>
            <div class="col-6 float-right">
                <button class="btn btn-sm btn-primary float-right mr-1" data-toggle="modal" data-target="#modalAddSPD">Delete
                    SPD</button>
                <button class="btn btn-sm btn-primary float-right mr-1" data-toggle="modal" data-target="#modalAddSPD">Edit SPD</button>
                <button class="btn btn-sm btn-primary float-right mr-1" data-toggle="modal" data-target="#modalAddSPD">Tambah
                        SPD</button>
            </div>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="mb-0 table table-sm table-striped table-bordered datatable ">
                    <thead>
                        <tr>
                            <th class="text-sm p-2 text-center m-0"><input type="checkbox" name="selectAll" id="selectAll"/></th>
                        ...
                        @foreach ($spd as $row)
                            <tr>
                                <td class="text-xxs p-2 m-0">
                                    <input type="checkbox" name="cb[]" id="select" value="{{$row->id_spd}}" data-spd="{{$row->no_spd}}">
                                </td>
                                <td class="text-xxs p-2 m-0">
                                    {{ $n++ }}
                                </td>
                                ...
                                <td class="text-xxs p-2 m-0">
                                    ...
                                    NRP: {{ $row->nrp }} <br>
                                    Pangkat/Golongan: {{ $row->personel->pangkat->nama_pangkat }} /
                                    {{ $row->personel->pangkat->golongan }}
                                </td>
                                <td class="text-xxs p-2 m-0">
...
@section('extra-modal')
    ...
    <!-- Modal Pengikut -->
    <div class="modal fade" id="modal-search-pengikut" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
    ...
    <script>
        ...
        function searchAnggota() {
            ...
            $.get("{{ route('get-personel-data') }}", {
                ...
                console.log(data);
                dataAnggota = "<tr><td>" + data.nama_personel + "</td><td>" + data.pangkat.nama_pangkat + "/" + data
                    .nrp + "</td><td>" + data.pangkat.golongan + "</td><td>" + data.status.nama_status +
                    "</td><td>" + data.jabatan + "</td><td>" + data.satker.nama_satker + "</td></tr>";
