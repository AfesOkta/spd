@extends('layout')
@section('page-title', 'Kwitansi Luar Negeri')
@section('page-title-desc', 'Data Kwitansi Luar Negeri')
@section('kwitansiln-menu', 'mm-active')
@section('content')
...
@section('extra-js')
@include('components.modal_pengeluaran_rill_ln')
...
<script>
	...
	function fill_add_kwitansi(no_spd){
		...
		// do ajax request
		$.get('{{route('get-kwitansi_ln-data')}}', {no_spd:no_spd}, function(data){
			$('#noSpd').val(no_spd)
	...
	function fill_edit_kwitansi(no_spd){
		...
		$.get('{{route('get-kwitansi_ln-data')}}', {no_spd:no_spd}, function(data){
            ...
            $('#deleteKwitansiBtn').attr('href', "{{url('/')}}"+"/delete_kwitansi_ln/id="+noSpd)
		});
	}
